...
import fs from 'fs';
import { InstallerPlugin, getAssetsDetails } from 'fbw-installer-plugins-plugins';
import { program } from 'commander';
import Logger from './lib/logger';

program
    .description('Verifies a FlyByWire Installer Plugin')
    .requiredOption('-p, --plugin <path>', 'Provide a path to the json file of the plugin to verify.')
    .action(async (options) => {
        const pluginFile = options.plugin;
        ...
        let plugin: InstallerPlugin;
        try {
            plugin = JSON.parse(fs.readFileSync(pluginFile, 'utf-8')) as InstallerPlugin;
        } catch (err) {
